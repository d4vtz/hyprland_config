...
Item {
    ...
    function closePanel() {
        audioMenu = 0
        open = false
        PanelCoordinator.close("controlcenter")
    }
    ...
    function requestBrightness(value) {
        pendingBrightness = Math.max(0.02, value)
        SystemStatus.brightness = pendingBrightness
    ...
    function setPowerProfile(profile) {
        ...
        if (SystemStatus.powerBackend === "tuned")
            profileSetter.command = ["pkexec", "tuned-adm", "profile", tunedProfiles[profile]]
        else
            profileSetter.command = ["powerprofilesctl", "set", profile]

        busy = true
        statusError = false
        statusMessage = "Cambiando perfil…"
        profileSetter.running = true
    }

    function deviceName(node, input) {
        if (!node)
            return input ? "Sin entrada" : "Sin salida"
    ...
    Timer {
        ...
        onTriggered: {
            brightnessSetter.command = [
                "brightnessctl",
                "set",
                Math.max(2, Math.round(root.pendingBrightness * 100)) + "%"
            ]
    ...
    PanelWindow {
        ...
        Surface {
            ...
            focus: root.open
            width: 440
            height: Math.min(700, parent.height - 76)
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        color: root.statusError
                            ? Qt.rgba(1, 0.33, 0.33, 0.14)
                            : Qt.rgba(0.31, 0.98, 0.48, 0.12)
                        ...
                        Row {
                            ...
                            OrionIcon {
                                materialName: root.busy
                                    ? "progress_activity"
                                    : root.statusError
                                        ? "error"
                                        : "check_circle"
                                fallbackColor: root.statusError ? Theme.red : Theme.green
                                size: 16
                            }
    ...
    Connections {
        ...
        function onActivePanelChanged() {
            if (root.open && PanelCoordinator.activePanel !== "controlcenter")
                root.closePanel()
        }
    }
}
