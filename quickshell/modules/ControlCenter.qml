...
Item {
    ...
    Process { id: command; onExited: function(exitCode, exitStatus) { root.busy = false; root.statusError = exitCode !== 0; root.statusMessage = exitCode === 0 ? "Cambio aplicado" : "No se pudo aplicar el cambio"; statusTimer.restart(); SystemStatus.refresh() } }
    Process { id: brightnessSetter; onExited: SystemStatus.refresh() }
    Process { id: externalLauncher }
    Process { id: profileSetter; onExited: function(exitCode, exitStatus) { root.busy = false; root.statusError = exitCode !== 0; root.statusMessage = exitCode === 0 ? "Perfil aplicado" : "No se pudo cambiar el perfil"; statusTimer.restart(); SystemStatus.refresh() } }
    ...
    PanelWindow {
        ...
        Surface {
            ...
            anchors.top: parent.top; anchors.right: parent.right; anchors.topMargin: Theme.barPanelTopMargin; anchors.rightMargin: 10
            elevated: true
            Behavior on height { NumberAnimation { duration: Theme.animationFast; easing.type: Easing.OutCubic } }
            MouseArea { anchors.fill: parent }
            Flickable {
                anchors.fill: parent; clip: true; contentWidth: width; contentHeight: contentColumn.implicitHeight + Theme.spacingLg * 2; boundsBehavior: Flickable.StopAtBounds
                ColumnLayout {
                    id: contentColumn
                    ...
                    Rectangle { visible: root.statusMessage.length > 0; Layout.fillWidth: true; Layout.preferredHeight: visible ? 30 : 0; radius: Theme.cardRadius; color: root.statusError ? Qt.rgba(1, 0.33, 0.33, 0.14) : Qt.rgba(0.31, 0.98, 0.48, 0.12); Row { anchors.centerIn: parent; spacing: 7; OrionIcon { name: root.busy ? "content-loading" : root.statusError ? "dialog-error" : "dialog-ok"; category: "status"; size: 16 }; Text { text: root.statusMessage; color: root.statusError ? Theme.red : Theme.green; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSizeSmall } } }
